...
def test_SourceConversationWrapper_on_conversation_updated(mocker, qtbot):
    file_ = factory.File(source=factory.Source(), is_downloaded=True)
...
def test_ConversationView_add_downloaded_file(mocker, homedir, source, session):
    ...
    controller = mocker.MagicMock(get_file=mocker.MagicMock(return_value=file))
...
def test_DeleteConversationAction_trigger(mocker):
    ...
    mocker.patch("securedrop_client.gui.conversation.DeleteDialog", mock_delete_conversation_dialog)
