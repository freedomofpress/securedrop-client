/**
 * Manages isolated SecureDrop server instances using Docker.
...
export class ServerInstance {
  ...
  private getSecuredropPath(): string {
    const possiblePaths = [
      resolve(__dirname, "../../../securedrop"), // Local: ../securedrop relative to securedrop-client
      resolve(__dirname, "../../securedrop-server"), // CI: securedrop-server checked out in workspace
    ];

    for (const p of possiblePaths) {
      if (existsSync(resolve(p, "securedrop", "manage.py"))) {
        return p;
      }
    }

    throw new Error(
      `Could not find SecureDrop server source. Tried: ${possiblePaths.join(", ")}`,
    );
  }
}
