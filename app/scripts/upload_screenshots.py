...
class WeblateClient:
    ...
    def associate_unit(self, screenshot_id: int, unit_id: int) -> bool:
        """
        Associate a unit with a screenshot.
        Returns True if association was created, False if it already existed.
        """
        ...
        if response.status_code == 400:
            # Check if this is a "already associated" error
            logger.debug(f"Response {response.status_code}: {response.text}")
            error_detail = response.json().get("unit_id", [])
            if any("already" in str(e).lower() for e in error_detail):
...
def main():
    ...
    if not weblate_url:
        logger.error("WEBLATE environment variable is required")
        sys.exit(1)

    api_token = os.environ.get("WEBLATE_API_TOKEN")
    if not api_token:
        logger.error("WEBLATE_API_TOKEN environment variable is required")
        sys.exit(1)
    ...
    if extra_cookies_json:
        try:
            ...
            logger.error(f"WEBLATE_EXTRA_COOKIES is not valid JSON: {e}")
            sys.exit(1)

    if not args.screenshots_dir.is_dir():
        logger.error(f"Screenshots directory not found: {args.screenshots_dir}")
        sys.exit(1)
    ...
    if unmatched:
        logger.warning(
            f"{len(unmatched)} screenshots have no matching translation units: {unmatched[:5]}..."
            if len(unmatched) > 5
            else f"{len(unmatched)} screenshots have no matching translation units: {unmatched}"
        )
