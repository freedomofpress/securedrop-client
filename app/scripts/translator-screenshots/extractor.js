...
class TranslationExtractor {
  ...
  /**
   * Get all keys for SignIn namespace
   */
  getSignInKeys(locale = "en") {
    return this.extractNamespaceKeys("SignIn", locale);
  }

  /**
   * Generate summary of translation keys
   */
  generateSummary(keys) {
    const summary = {
      totalKeys: Object.keys(keys).length,
      keysWithInterpolation: 0,
      keysList: [],
    };

    for (const [shortKey, data] of Object.entries(keys)) {
      if (data.hasInterpolation) {
        summary.keysWithInterpolation++;
      }

      summary.keysList.push({
        key: shortKey,
        fullKey: data.fullKey,
        text: data.processedText,
        hasInterpolation: data.hasInterpolation,
      });
    }
...
// CLI usage (for debugging/testing) - ESM equivalent
if (import.meta.url === `file://${process.argv[1]}`) {
  const extractor = new TranslationExtractor();

  try {
    const signInKeys = extractor.getSignInKeys();
    const summary = extractor.generateSummary(signInKeys);

    console.log("=== SignIn Translation Keys ===");
    console.log(`Total keys: ${summary.totalKeys}`);
    console.log(`Keys with interpolation: ${summary.keysWithInterpolation}`);
    console.log("\n=== Key Details ===");

    summary.keysList.forEach((item) => {
      console.log(`${item.key}:`);
      console.log(`  Text: "${item.text}"`);
      if (item.hasInterpolation) {
        console.log(`  (has interpolation)`);
      }
      console.log();
    });
  } catch (error) {
