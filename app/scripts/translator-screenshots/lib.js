...
class TranslationScreenshotAPI {
  ...
  /**
   * Hover over an element by test ID (useful for tooltips)
   * @param {string} testId - data-testid attribute value, use * for wildcard suffix
   */
  async hoverByTestId(testId) {
    console.log(`Hovering over element with testid: ${testId}`);
  ...
  /**
   * Find element by text (reused from original code)
   */
  ...
  async _findElementByTextExact(text) {
    ...
    try {
      const element = await this.page.locator(`xpath=${bestCandidate.xpath}`);
  ...
  /**
   * Get summary of screenshot state
   */
  getState() {
    return {
      context: this.currentContext,
      screenshotsTaken: Array.from(this.screenshotState),
      totalTaken: this.screenshotState.size,
    };
  }

  /**
   * Reset screenshot state (useful for testing)
   */
  resetState() {
    this.screenshotState.clear();
    console.log("Screenshot state reset");
  }
