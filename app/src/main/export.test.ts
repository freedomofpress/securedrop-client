...
describe("ArchiveExporter", () => {
  ...
  it("should throw ExportError for unknown status", () => {
    const exporter = new TestExporter();
    expect(() => exporter.parseStatus("foo\nBAR")).toThrow(ExportError);
  });

  it("should throw ExportError for empty output", () => {
    const exporter = new TestExporter();
    expect(() => exporter.parseStatus("")).toThrow(ExportError);
  });
