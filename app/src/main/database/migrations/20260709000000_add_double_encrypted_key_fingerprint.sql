-- migrate:up
-- Primary fingerprint of the key that decrypted the inner layer of a
-- double-encrypted item, set only when it differs from the submission key
-- that was current at decryption time (e.g. a rotated key still in sd-gpg).
ALTER TABLE items ADD COLUMN double_encrypted_key_fingerprint TEXT;
