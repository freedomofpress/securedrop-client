...
function KeyboardHelp() {
  ...
  const shortcuts = [
    ...
    },
    {
      heading: 'Application',
      items: [
        { desc: 'Quit SecureDrop', keys: [['Ctrl', 'Q']] },
      ],
    },
  ];
      ...
      <span key={ci} className="flex items-center gap-[4px]">
        {chord.map((key, i) => (
          <span key={i} className="flex items-center gap-[4px]">
            ...
            {i < chord.length - 1 && <span className="text-[11px] text-[#8c8c8c] leading-none">+</span>}
          </span>
        ))}
      </span>
    ));
  ...
  return (
    <>
      ...
      <div className="flex items-center gap-[8px] px-[24px] pt-[16px] pb-[12px] border-b border-[#f0f0f0]">
        {/* Keyboard icon */}
        <span className="text-[16px] font-semibold text-[#181d27]">{ t("help.keyboardShortcutsTitle")}</span>
      </div>
      ...
      <div className="px-[24px] pt-[16px] pb-[4px]">
        {shortcuts.map((group, gi) => (
          <div key={gi}>
            <div className={`text-[11px] font-bold uppercase tracking-[0.06em] text-[#8c8c8c] mb-[2px] ${gi > 0 ? 'mt-[4px]' : ''}`}>
              {group.heading}
            </div>
            {group.items.map((item, ii) => (
              <div key={ii} className="flex items-center justify-between py-[5px]">
            ...
            {gi < shortcuts.length - 1 && <div className="h-[1px] bg-[#f0f0f0] my-[8px]"></div>}
          </div>
        ))}
      </div>
    </>
