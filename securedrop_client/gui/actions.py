...
class ExportConversationAction(QAction):
    """Export all messages and replies of the currently selected conversation."""

    def __init__(
        self, parent: QMenu, controller: Controller, app_state: Optional[state.State], confirmation_dialog,
    ) -> None:
        self._controller = controller
        self._state = app_state
        self._text = _("Conversation")
        super().__init__(self._text, parent)


        self._confirmation_dialog = confirmation_dialog(controller, "123", "spike")
        self._confirmation_dialog.accepted.connect(
            lambda: self.controller.export_conversation("123")
        )
        ...
        if self._controller.api is None:
            ...
            if self._controller.api is None:
                ...
                if self._state is not None:
                    ...
                    if id is None:
                        return
                    # self._controller.export_conversation(id)
