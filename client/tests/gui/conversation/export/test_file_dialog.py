...
def test_ExportDialog_init(mocker):
    ...
    export_file_dialog = ExportDialog(mocker.MagicMock(), "mock.jpg", ["/mock/path/to/file"])
...
def test_ExportDialog_init_sanitizes_filename(mocker):
    secure_qlabel = mocker.patch(
        "securedrop_client.gui.conversation.export.export_dialog.SecureQLabel"
    )
    mocker.patch("securedrop_client.gui.widgets.QVBoxLayout.addWidget")
    filename = '<script>alert("boom!");</script>'

    ExportDialog(mocker.MagicMock(), filename, ["/mock/path/to/file"])
...
# The summary text is different for a dialog with one file vs 3 files
def test_ExportDialog__show_starting_instructions_single_file(mocker, export_dialog):
...
def test_ExportDialog__export_file(mocker, export_dialog_multifile):
    ...
    device.export.assert_called_once_with(export_dialog_multifile.filepaths, "mock_passphrase")
...
def test_ExportDialog__on_export_preflight_check_succeeded_device_unlocked(
    ...
    export_dialog_multifile._export.assert_not_called()
    export_dialog_multifile.continue_button.clicked.connect.assert_called_once_with(
        export_dialog_multifile._export
    )
