...
def test_FileWidget_on_left_click_download(mocker, session, source):
    ...
    assert controller.on_submission_download.call_args[0][0] == db.File
    assert controller.on_submission_download.call_args[0][1] == file_.uuid
    # ProgressProxy is created dynamically so just assert the type
    assert isinstance(controller.on_submission_download.call_args[0][2], ProgressProxy)
...
def test_SourceConversationWrapper_on_conversation_updated(mocker, qtbot):
    ...
    expected_timestamp = "Last seen: {thetime}".format(
            thetime=format_datetime_local(source.last_updated, True)
            )
...
def test_SourceProfileShortWidget_update_timestamp(mocker):
    ...
    spsw.updated.setText.assert_called_once_with("Last seen: {thetime}".format(
        thetime=format_datetime_local(mock_source.last_updated, long_format=True)
        )
                                                 )
